...
export default defineConfig({
  ...
  build: {
    rollupOptions: {
      output: {
        /**
         * FIX: Agrupa React + ReactDOM en un único chunk para evitar
         * la inicialización circular que causaba dependencias circulares
         * en el grafo de módulos.
         */
        manualChunks(id) {
          if (!id.includes("node_modules")) return undefined;
          // Firebase va solo — es grande (411KB) y no inicia con React
          if (id.includes("firebase")) return "vendor-firebase";
          // TODO el resto (react, react-dom, tanstack, lucide, etc.) va junto
          // → sin riesgo de que React esté undefined cuando otro módulo arranca
          return "vendor";
        },
      },
    },
