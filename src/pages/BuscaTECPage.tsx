...
const MOCK_RESULTS: SearchResult[] = [
  {
    ...
    subtitle: "Ing. Sistemas · Anual · 2.° año · 6 comisiones",
    badge: "Disponible",
    badgeColor: "green",
    iconName: "calculator",
  },
  {
    type: "aula",
    title: "Aula 304 — 3.° piso",
    subtitle: "Medrano · Ala norte · Lun y Mié 18hs",
    badge: "Comisión A",
    badgeColor: "blue",
    iconName: "map-pin",
  },
  {
    type: "grupo",
    title: "Grupo AM2 — Comisión A",
    subtitle: "47 miembros · Activo hoy",
    badge: "Unirme",
    badgeColor: "green",
    iconName: "users",
  },
  {
    type: "recurso",
    title: "GuíaTEC — AM2 desde cero",
    subtitle: "Video · Jairo T. · 45 min",
    badge: "Gratis",
    badgeColor: "green",
    iconName: "video",
  },
        ...
        <div className="flex-1 flex items-center gap-2 bg-itec-card border border-white/7 rounded-xl px-4 py-3 focus-within:border-itec-blue-skye transition-colors">
          ...
          {query && (
            <button
              ...
              className="text-[#5a6475] hover:text-itec-text transition-colors text-lg leading-none"
            >
              ×
            </button>
          )}
      ...
      {!searched ? (
        <div className="flex flex-col items-center justify-center py-16 text-center gap-3">
          ...
          <p className="text-[#5a6475] text-sm">
            Escribí algo arriba para buscar
            <br />
            materias, aulas, grupos y más
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          <p className="text-xs text-[#5a6475]">
            Resultados para{" "}
            <span className="text-itec-text font-medium">"{query}"</span>
          </p>
          {filtered.map((r, i) => (
            <div
              key={i}
            ...
            >
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${TYPE_ICON_BG[r.type]}`}
              >
                <div className="w-5 h-5">
                  <Icons type={r.iconName} className="w-full h-full" />
                </div>
              </div>
              ...
              {r.badge && (
                <span
                  className={`text-[10px] font-medium px-2 py-1 rounded-md shrink-0 ${BADGE_CLASSES[r.badgeColor ?? "blue"]}`}
                >
                  {r.badge}
                </span>
              )}
