...
export type HeaderColorTheme = 
  | 'purple' | 'orange' | 'blue' | 'course' | 'green' | 'yellow' 
...
export const PageHeader: React.FC<Props> = ({ 
  ...
  const themeStyles: Record<HeaderColorTheme, { box: string; glow: string }> = {
    course:  { box: 'bg-itec-blue-skye/10 text-itec-blue-skye border-itec-blue-skye/20 ', glow: 'rgba(0, 74, 173, 0.4)' },
    ...
    <header className="mb-6 md:mb-8 animate-in fade-in slide-in-from-top-2 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 md:gap-6">
        <div className="flex items-start gap-3 md:gap-4">
          {imageUrl ? (
            <img src={imageUrl} alt={title} className="w-12 h-12 md:w-16 md:h-16 object-contain shrink-0 mt-1" />
          ) : (
          ...
          <div className="flex flex-col pt-0.5">
            <h1 className="text-2xl md:text-3xl font-bold text-itec-text tracking-tight mb-1">{title}</h1>
            <p className="text-xs md:text-sm text-itec-gray max-w-xl leading-relaxed">{description}</p>
          </div>
        </div>
        {children && (
