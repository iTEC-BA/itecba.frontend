  ...
  return (
    <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
      <div className="flex flex-col gap-4">
        ...
        {actions.map(action => (
          <a key={action.id} href={action.url} target="_blank" rel="noopener noreferrer" className="bg-itec-box border border-itec-border/50 hover:border-itec-blue-skye p-4 rounded-xl flex items-center justify-between group transition-colors">
            <div>
        ...
        {siuLinks.map(siu => (
           <a key={siu.id} href={siu.url} target="_blank" rel="noopener noreferrer" className="bg-itec-box border border-itec-border/50 hover:border-itec-blue-skye p-4 rounded-xl flex items-center justify-between group transition-colors">
           <div>
      ...
      <div className="flex flex-col gap-4">
        ...
        {materials.map(mat => (
          <a key={mat.id} href={mat.url} target="_blank" rel="noopener noreferrer" className="bg-itec-box border border-itec-border/50 hover:border-orange-400 p-4 rounded-xl flex items-center gap-4 group transition-colors">
        ...
        <div className="grid grid-cols-3 gap-2 h-full">
          {socials.map(soc => (
             <a key={soc.id} href={soc.url} target="_blank" rel="noopener noreferrer" className={`bg-itec-box border border-itec-border/50 ${soc.hoverClass} p-3 rounded-xl flex flex-col items-center justify-center text-center group transition-colors h-full min-h-[80px]`}>
