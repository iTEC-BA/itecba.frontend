      ...
      <div className="relative border-l-2 border-itec-gray/50 ml-4 space-y-6 pb-2 flex-1">
        {steps.map((step) => {
          ...
          return (
            ...
            >
              {/* Círculo Interactivo */}
              <span className={`absolute -left-[11px] top-1 w-5 h-5 rounded-full border-4 border-itec-box flex items-center justify-center transition-all duration-300 ${
                isCompleted 
                ...
                {isCompleted && <span className="text-itec-texttext-[8px] font-bold">✓</span>}
              </span>

              {/* Tarjeta del Paso */}
              <div className={`transition-all duration-300 ${isCompleted ? 'opacity-50 hover:opacity-80' : 'opacity-100'}`}>
                <h3 className={`text-sm font-bold mb-1 transition-colors ${isCompleted ? 'text-itec-text line-through' : 'text-itec-textgroup-hover:text-purple-300'}`}>
                ...
                <p className={`text-xs leading-relaxed p-3 rounded-xl border transition-all duration-300 ${
                  isCompleted 
                    ...
                    : 'bg-itec-bg/50 border-itec-gray/30 text-itec-text group-hover:border-purple-500/30'
                }`}>
                  {step.description}
                </p>
              </div>
