      ...
      <div className="mb-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
          {materials.map(material => (
            <a key={material.id} href={material.url} target="_blank" rel="noopener noreferrer" className="bg-itec-box border border-itec-border/50 hover:border-purple-500/60 rounded-2xl p-6 text-center transition-all duration-300 group hover:-translate-y-1.5 shadow-lg overflow-hidden relative">
              ...
              <div className="w-16 h-16 mx-auto bg-itec-bg border border-itec-border/50 rounded-2xl mb-4 flex items-center justify-center text-3xl group-hover:scale-110 transition-transform duration-500 group-hover:border-purple-500/40 shadow-inner relative z-10 -rotate-3 group-hover:rotate-0">
                {material.emoji}
              </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {siuLinks.map((siu, index) => (
            <a key={siu.id} href={siu.url} target="_blank" rel="noopener noreferrer" className="bg-gradient-to-br from-itec-box to-[#080b0f] border border-itec-border/50 p-6 rounded-2xl flex items-center gap-5 hover:border-blue-500/50 transition-all duration-300 group shadow-xl hover:-translate-y-1">
