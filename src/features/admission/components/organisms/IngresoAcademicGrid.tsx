      ...
      <div className="mb-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {materials.map(material => (
            <a key={material.id} href={material.url} target="_blank" rel="noopener noreferrer" className="bg-itec-box border border-itec-gray hover:border-purple-500/50 rounded-3xl p-6 text-center transition-all duration-300 group hover:-translate-y-1 shadow-lg overflow-hidden relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {siuLinks.map((siu, index) => (
            <a key={siu.id} href={siu.url} target="_blank" rel="noopener noreferrer" className="bg-gradient-to-r from-gray-900 to-black border border-gray-800 p-6 rounded-3xl flex items-center gap-5 hover:border-blue-500/50 transition-all duration-300 group shadow-xl">
              <div className="w-14 h-14 bg-blue-500/10 border border-blue-500/20 rounded-2xl flex items-center justify-center text-blue-400 shrink-0 group-hover:scale-105 transition-transform group-hover:bg-blue-500 group-hover:text-itec-textshadow-inner">
