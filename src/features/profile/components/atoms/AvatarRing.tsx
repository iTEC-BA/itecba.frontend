...
export const AvatarRing: React.FC<AvatarRingProps> = ({
  src, name, size = "md", glow = false, online, className, ring,
}) => {
    ...
    <div className={cn("relative shrink-0", className)}>
      {glow && (
        <div className={cn(
          ...
          "bg-gradient-to-br from-itec-sky/30 to-itec-purple/30 blur-sm scale-105"
        )} />
      )}
      <div className={cn(
        "relative rounded-full overflow-hidden",
        "flex items-center justify-center font-black text-itec-muted",
        ring ? ring : "border-2 border-white/15 bg-gradient-to-br from-itec-box2 to-itec-surface",
        s.ring
      )}>
        {src ? (
          <img
      ...
      {online !== undefined && (
        <span className={cn(
          "absolute bottom-0 right-0 rounded-full border-itec-box",
          s.dot,
