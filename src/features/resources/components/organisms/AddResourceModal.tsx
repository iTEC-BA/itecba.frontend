  ...
  return (
    ...
    <div className="fixed inset-0 z-100 flex items-end sm:items-center justify-center bg-black/80  p-0 sm:p-4">
      <div className="w-full sm:max-w-lg bg-itec-box border border-itec-gray/30 rounded-t-3xl sm:rounded-xl shadow-2xl shadow-black/50 flex flex-col max-h-[92dvh] sm:max-h-[90vh]">
        ...
        {/* Header fijo */}
        <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-itec-border/50 shrink-0">
          <div>
                    ...
                    <ul className="absolute z-50 top-full mt-1 w-full max-h-40 overflow-y-auto bg-itec-sidebar border border-itec-gray/40 rounded-xl shadow-2xl shadow-black/40">
                      {filteredOptions.map(m => (
                        <li key={m} onMouseDown={() => handleSelectMateria(m)}
                          className="px-3 py-2.5 text-sm text-itec-text hover:bg-orange-600/80 cursor-pointer border-b border-itec-border/50 last:border-0 transition-colors truncate">
                          {m}
                        </li>
                      ))}
                    </ul>
