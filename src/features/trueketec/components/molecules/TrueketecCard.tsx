...
export const TrueketecCard: React.FC<Props> = ({ post, onContact }) => {
  ...
  return (
    <article className={cn(
      "flex flex-col rounded-2xl p-4 gap-4 transition-colors duration-200 h-full",
      isPerfect ? "bg-itec-blue-skye/10" : "bg-itec-box hover:bg-itec-surface",
      isCompleted && "opacity-50 grayscale"
    )}>
      
      <div className="flex flex-col gap-2">
        <div className="flex items-start justify-between gap-2">
          <span className={cn("text-[10px] font-bold uppercase tracking-widest", isPerfect ? "text-itec-blue-skye" : "text-itec-muted")}>
            {post.departamento}
          </span>
          {isPerfect ? (
            <span className="px-2 py-0.5 rounded bg-itec-blue-skye text-black text-[9px] font-bold uppercase tracking-widest shrink-0 flex items-center gap-1">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 mt-auto bg-itec-bg/50 rounded-xl p-3">
        ...
        <div className="flex flex-col gap-1 text-center">
          ...
          <span className={cn("font-mono text-sm font-bold", isPerfect ? "text-itec-blue-skye" : "text-white")}>
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        {onContact && !isCompleted && (
          <Button
            ...
            className={cn("py-2 px-4 text-[10px] uppercase tracking-wider rounded-lg", isPerfect ? "bg-itec-blue-skye text-black hover:bg-itec-blue" : "bg-itec-surface text-itec-text border-transparent hover:bg-itec-bg")}
          />
        )}
      </div>
    </article>
