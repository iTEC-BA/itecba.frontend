    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-44 rounded-xl border border-itec-border/50 bg-itec-box animate-pulse" />
        ))}
      </div>
    );
  }
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-2">
          <button
            ...
            className="flex items-center gap-1 rounded-xl border border-itec-border/50 px-3 py-2 text-sm text-itec-muted transition-colors hover:bg-itec-surface disabled:opacity-30"
          >
