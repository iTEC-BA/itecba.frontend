  ...
  return (
    <LayoutModal isOpen={!!postulante} onClose={onClose} title={`Expediente: ${postulante.userName}`} description="Documentación vinculada al alumno." maxWidth="max-w-md">
      <div className="flex flex-col gap-5 px-6 py-6">
        <a href={`mailto:${postulante.userEmail}`} className="flex items-center gap-3 text-sm font-mono text-white hover:underline pb-5 border-b border-itec-border/50">
          <div className="p-2.5 bg-itec-surface border border-itec-border/50 rounded-xl"><Mail size={16} className="text-itec-section-trueketec" /></div>
          {postulante.userEmail}
        </a>

        {postulante.ofertas.length === 0 ? (
          <div className="bg-itec-surface rounded-xl border border-itec-border/50 p-8 text-center text-[10px] uppercase tracking-widest font-bold text-white/30">0 Registros Activos</div>
        ) : (
          <div className="flex flex-col gap-4">
            ...
            {postulante.ofertas.map((oferta) => (
              <div key={oferta._id} className="flex flex-col gap-4 rounded-xl border border-itec-border/50 bg-itec-box p-5">
                <div className="flex items-start gap-3 border-b border-itec-border/50 pb-3">
                ...
                <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 bg-itec-surface border border-itec-border/50 p-3 rounded-lg">
