...
export const FAQAdminPanel: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  const handleSaveFAQ = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : "Error al guardar la FAQ");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDeleteFAQ = async (id: string) => {
    ...
    await remove(id).catch((e: unknown) =>
      alert(e instanceof Error ? e.message : "Error al eliminar la FAQ"),
    );
  };
  ...
  const handleSaveContext = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        err instanceof Error ? err.message : "Error al guardar el contexto",
      );
    } finally {
      setSaving(false);
    }
                    ...
                    <div className="flex gap-2 pt-1">
                      {editing && (
                        <button
                          type="button"
                          onClick={cancelEdit}
                          className="px-4 py-2 rounded-xl text-xs font-bold bg-white/5 text-white/50 hover:text-white transition-colors"
                        >
                          Cancelar
                        </button>
                      )}
                      <button
                        type="submit"
                        disabled={saving}
                        className="flex-1 py-2 rounded-xl text-xs font-bold bg-white text-black hover:bg-white/90 transition-colors disabled:opacity-50"
                      >
                        {saving
                          ? "Guardando..."
                          : editing
                ...
                <form onSubmit={handleSaveContext} className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      rows={5}
                      className={`${inputCls} resize-none`}
                      placeholder="UTN FRBA es..."
                      value={ctxForm.institutionalContext ?? ""}
                      onChange={(e) =>
                        setCtxForm((p) => ({
                          ...p,
                          institutionalContext: e.target.value,
                        }))
                      }
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      rows={4}
                      className={`${inputCls} resize-none`}
                      placeholder="Responde solo sobre temas de UTN..."
                  ...
                  <button
                    type="submit"
                    disabled={saving}
                    className="w-full py-3 rounded-2xl text-sm font-bold bg-white text-black hover:bg-white/90 transition-colors disabled:opacity-50"
                  >
