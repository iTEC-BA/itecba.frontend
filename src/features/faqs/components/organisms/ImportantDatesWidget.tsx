...
export const ImportantDatesWidget: React.FC<Props> = ({ isAdmin }) => {
  ...
  return (
    <section className="bg-itec-box border border-itec-gray rounded-3xl p-6 shadow-2xl h-full animate-in fade-in duration-500">
      <div className="flex items-center justify-between mb-8">
        ...
        {isAdmin && (
           <button onClick={() => setIsModalOpen(true)} className="py-1 px-2 flex items-center justify-center rounded-full bg-itec-bg border border-itec-gray text-itec-text hover:text-itec-texthover:border-orange-500 transition-colors cursor-pointer" title="Agregar Fecha">
        ...
        <div className="relative border-l border-itec-gray/50 ml-3 space-y-8 pb-4">
          {activeDates.map((item, index) => (
            <div key={item.id} className="relative pl-6 group">
              <span className={`absolute -left-[5.5px] top-1.5 w-2.5 h-2.5 rounded-full border-2 border-itec-box transition-colors ${index === 0 ? 'bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.8)]' : 'bg-gray-500 group-hover:bg-orange-400'}`}></span>
              <div>
                <p className="text-[10px] text-orange-400 font-bold uppercase tracking-widest mb-1">{item.date}</p>
                <h3 className="text-sm font-bold text-itec-textmb-1.5 leading-snug">{item.title}</h3>
                <p className="text-[12px] text-itec-text leading-relaxed bg-itec-bg/50 p-2.5 rounded-lg border border-itec-gray/30">{item.description}</p>
              </div>
            </div>
          ))}
        </div>
