      ...
      >
        ...
        <div className="flex items-start justify-between px-5 py-4 border-b border-white/6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <span className={`inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-full border mb-1.5 ${labelCls}`}>
                {labelText}
              </span>
              <h2 className="text-sm font-bold text-itec-text leading-snug">{item.title}</h2>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center gap-2 px-5 py-2.5 border-b border-white/6">
          <Calendar className="size-3.5 text-itec-text/30" />
          <span className="text-[11px] text-itec-text/40 capitalize">{date}</span>
        </div>
