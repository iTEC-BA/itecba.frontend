...
export const GroupGlowCard: React.FC<Props> = ({ children, className = '', onClick, glowColor = 'green' }) => {
  ...
  return (
    <Tag
      onClick={onClick}
      className={`bg-itec-box border border-itec-border/50 rounded-xl transition-all duration-200 ${GLOW[glowColor]} ${onClick ? 'cursor-pointer text-left w-full' : ''} ${className}`}
    >
      {children}
    </Tag>
  );
