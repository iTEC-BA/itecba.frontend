// src/features/groups/components/organisms/AddGroupModal.tsx
// Modal principal para aportar un grupo de WhatsApp.
// Los admins pueden abrir AddMateriaInlineModal si la materia no existe.
...
import { AddMateriaInlineModal } from "../molecules/AddMateriaInlineModal";
...
export const AddGroupModal: React.FC<Props> = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!form.link.startsWith("http")) {
      setError("El link debe comenzar con https://");
      return;
    }

    try {
      const isDupApproved = existingGroups.some(
        (g) =>
          (g.materia === form.materia &&
            g.comision.toLowerCase() === form.comision.toLowerCase()) ||
          g.link === form.link,
      );
      if (isDupApproved) {
        setError(
          `Este link o la comisión ${form.comision.toUpperCase()} ya están registrados.`,
        );
        return;
      }

      const isDupPending = await groupsService.checkIsDuplicatePending(
        form.materia,
        form.comision,
        form.link,
      );
      if (isDupPending) {
        setError("Este grupo ya fue sugerido y está en revisión.");
        return;
      }
      ...
      submitMutation.mutate(
        {
          ...
          isDirectPublish: isAuthenticated,
        },
        {
          onSuccess: () => {
            setSuccessInfo({
              title: isAuthenticated ? "¡Grupo Publicado!" : "¡Solicitud Enviada!",
              desc: isAuthenticated
                ? "Tu grupo ya está visible para todos."
  ...
  /** Callback cuando se crea una nueva materia desde el sub-modal. */
  const handleMateriaCreated = (nombreMateria: string) => {
    setForm((f) => ({ ...f, materia: nombreMateria }));
    setShowDropdown(false);
  };
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/6 shrink-0">
            <div>
              ...
              <p className="text-[11px] text-itec-gray mt-0.5">
                Completá los datos para sumarlo a la comunidad.
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-5">
            {success ? (
              <div className="text-center py-12 animate-in fade-in duration-300 flex flex-col items-center gap-3">
                <div className="w-14 h-14 bg-itec-groups/15 border border-itec-groups/30 rounded-full flex items-center justify-center text-2xl">
                  ✅
                </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="bg-itec-blue-skye/10 border border-itec-blue-skye/25 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-bold text-itec-text">
                        Iniciá sesión para publicar directo
                      </p>
                      <p className="text-xs text-itec-gray mt-0.5">
                        Sin revisión y sumás puntos a tu TarjeTEC.
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={loginWithGoogle}
                      className="shrink-0 flex items-center gap-2 bg-white hover:bg-gray-100 text-gray-900 text-xs font-bold py-2 px-4 rounded-lg transition-colors shadow-sm"
                    >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold text-itec-gray uppercase tracking-wider mb-1.5">
                      1. Especialidad
                    </label>
                    <Select
                      fullWidth
                ...
                <div ref={dropdownRef} className="relative">
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="block text-[10px] font-bold text-itec-gray uppercase tracking-wider">
                      3. Materia
                    </label>
                    {/* Botón "agregar materia" solo para admins */}
                    {isAdmin && form.carrera && form.nivel && (
                      <button
                        type="button"
                  ...
                  <Input
                    ...
                    placeholder={
                      !form.carrera || !form.nivel
                        ? "Seleccioná especialidad y nivel..."
                        : loadingMaterias
                          ? "Cargando..."
                          : "Escribí para buscar..."
                    }
                ...
                {/* Tipo de grupo (solo admin) */}
                {isAdmin && (
                  <div className="bg-itec-blue-skye/5 border border-itec-blue-skye/15 rounded-xl p-3.5">
                    <label className="block text-[10px] font-bold text-itec-blue-skye uppercase tracking-wider mb-1.5">
                      Admin — Tipo de grupo
                    </label>
                    <Select
                      fullWidth
                      options={[
                        { value: "Alumnos", label: "Grupo de Alumnos" },
                        { value: "Oficial", label: "Grupo Oficial (ITEC)" },
                      ]}
                      value={form.tipo}
                      onChange={(e) =>
                        setForm({
                ...
                {error && (
                  <div className="bg-itec-red/10 border border-itec-red/25 p-3 rounded-xl flex items-start gap-2.5">
                    <Icons
                      type="info"
                      className="w-4 h-4 text-itec-red shrink-0 mt-0.5"
                    />
                    <p className="text-itec-red text-xs leading-relaxed">
                      {error}
                    </p>
                  </div>
                )}
                ...
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  <Button
                    ...
                    variant="primary"
                    disabled={submitMutation.isPending}
                    className="text-xs bg-itec-groups hover:bg-emerald-500 border-none shadow-md min-w-28 active:scale-95"
                  >
      ...
      {/* Sub-modal para crear nueva materia (solo admins) */}
      {isAddMateriaOpen && (
        <Suspense fallback={null}>
          <AddMateriaInlineModal
            isOpen={isAddMateriaOpen}
            initialCarrera={form.carrera}
            initialNivel={form.nivel}
            onClose={() => setIsAddMateriaOpen(false)}
            onCreated={handleMateriaCreated}
          />
        </Suspense>
      )}
    </>
