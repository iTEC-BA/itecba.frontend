...
export const SubjectCard: React.FC<Props> = ({ subject }) => {
  ...
  const statusConfig = {
    ...
    cursando: { bg: 'bg-yellow-400/10', border: 'border-yellow-400/50', badge: 'bg-yellow-500 text-itec-textanimate-pulse', text: 'text-yellow-500', icon: 'M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z' },
    disponible: { bg: 'bg-itec-box', border: 'border-itec-gray hover:border-itec-blue/50', badge: 'bg-itec-blue text-white', text: 'text-itec-blue-skye', icon: 'M5 12h14M12 5v14' },
      ...
      <div className="flex flex-col space-y-1.5 p-6">
        <div className="flex justify-between items-start gap-4">
          <div className="flex-1">
            ...
            <div className="text-itec-text text-xs font-mono font-bold opacity-80">{subject.code}</div>
          </div>
          ...
          <div className={`animate-in fade-in slide-in-from-top-3 duration-500 p-3 rounded-xl border ${config.bg} ${config.border} grid grid-cols-2 items-start gap-4`}>
            {subject.status === 'aprobada' ? (
              <div className="flex flex-col gap-1.5">
                ...
                <input className={`w-full h-9 bg-itec-bg border ${config.border} rounded-lg text-center text-sm font-bold text-itec-textoutline-none focus:border-itec-blue`} type="number" defaultValue={subject.grade} readOnly />
              </div>
            ) : <div></div>}
            
            <div className="flex flex-col gap-1.5 items-end">
              <span className={`text-[10px] font-bold ${config.text}`}>AÑO</span>
              <input className={`w-full h-9 bg-itec-bg border ${config.border} rounded-lg text-center text-sm font-bold text-itec-textoutline-none focus:border-itec-blue`} type="number" defaultValue={subject.year} readOnly />
            </div>
          </div>
        </div>
      )}
