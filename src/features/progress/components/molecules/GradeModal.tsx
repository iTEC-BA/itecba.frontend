...
export const GradeModal: React.FC<Props> = ({ subject, targetStatus, onClose, onConfirm }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-itec-box border border-itec-gray rounded-2xl w-full max-w-sm p-6 shadow-2xl">
        <h3 className="text-xl font-bold text-itec-textmb-1">
          {isAprobada ? 'Aprobar / Promocionar' : 'Regularizar Materia'}
        </h3>
        <p className="text-sm text-itec-text mb-6">{subject.name}</p>
          ...
          <div className="flex gap-4">
            ...
            {isAprobada && (
              <div className="flex-1">
                <label className="block text-xs font-bold text-itec-text mb-1">Nota Final</label>
                <input 
                  type="number" min="1" max="10" required
                  value={grade} onChange={(e) => setGrade(Number(e.target.value))}
                  className="w-full bg-itec-bg border border-itec-gray rounded-lg px-3 py-2 text-itec-textoutline-none focus:border-itec-blue"
                  placeholder="Ej: 8"
                />
              </div>
            )}
            <div className="flex-1">
              <label className="block text-xs font-bold text-itec-text mb-1">Año de Cursada</label>
              <input 
