...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  event: CalendarEvent | null; // null = crear, definido = editar
...
const EMPTY: CalendarEventInput = {
  title: "", description: "", subtitle: "", date: "", type: "examen",
};

export const CalendarAdminModal: React.FC<Props> = ({
  isOpen, onClose, event, onCreate, onUpdate, onDelete,
}) => {
  ...
  const handleDelete = async () => {
    if (!event) return;
    if (!confirm("¿Eliminar esta fecha definitivamente?")) return;
    setDeleting(true);
    try {
    ...
    <LayoutModal
      ...
      description={
        isEditing
          ...
          : "Completá los campos para agregar una nueva fecha al calendario."
      }
      maxWidth="max-w-lg"
    >
      <div className="p-6 flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] font-bold text-[#5b9cf6] uppercase tracking-widest">
            Título *
          </label>
          <Input
            fullWidth
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] font-bold text-[#5b9cf6] uppercase tracking-widest">
            Subtítulo <span className="opacity-50 normal-case font-normal">(opcional)</span>
          </label>
          <Input
            fullWidth
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] font-bold text-[#5b9cf6] uppercase tracking-widest">
            Descripción <span className="opacity-50 normal-case font-normal">(opcional)</span>
          </label>
          <textarea
            rows={3}
            ...
            className="
              ...
              placeholder:text-itec-muted/80 outline-none transition-all resize-none w-full
        ...
        {/* ── Acciones ── */}
        <div className={`flex gap-2 pt-1 ${isEditing ? "justify-between" : "justify-end"}`}>
          {isEditing && (
            <Button
              variant="danger"
              hierarchy="outline"
          ...
          <div className="flex gap-2">
            ...
            <Button
              variant="primary"
              hierarchy="solid"
              text={saving ? "Guardando..." : isEditing ? "Guardar cambios" : "Crear fecha"}
              onClick={handleSave}
              disabled={saving || deleting}
              isLoading={saving}
            />
