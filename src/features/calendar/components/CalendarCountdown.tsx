...
export const CalendarCountdown: React.FC<Props> = ({ event, typeConfig }) => {
  ...
  return (
    <div className={`mb-6 rounded-2xl border p-4 ${cfg.color}`}>
      {/* Header */}
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {event.subtitle && (
            <p className="text-xs opacity-60 mt-0.5 truncate">{event.subtitle}</p>
          )}
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          ...
          <div
            key={label}
            className="flex flex-col items-center bg-black/20 rounded-xl py-2.5"
          >
            <span className="text-2xl font-bold text-itec-text tabular-nums leading-none">
              {pad(val)}
            </span>
