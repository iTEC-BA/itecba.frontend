...
const TYPE_CONFIG: Record<EventType, { label: string; color: string; dot: string }> = {
  ...
  feriado: { label: "Feriado", color: "bg-white/8 text-[#9aa3b0] border-itec-border/50", dot: "bg-[#9aa3b0]" },
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filtered.length === 0 && (
                <div className="col-span-full py-12 text-center text-sm text-[#5a6475] border border-dashed border-itec-border/50 rounded-xl">
              ...
              {filtered.map(ev => {
                ...
                return (
                  <div 
                    ...
                    className={`flex items-center gap-3 bg-itec-card border border-itec-border/50 rounded-xl p-4 cursor-pointer hover:border-itec-border/50 hover:bg-white/[0.02] transition-colors ${isAdmin ? "hover:border-itec-red/30" : ""}`}
                  >
                    <div className="flex flex-col items-center justify-center w-14 h-14 rounded-xl bg-black/20 shrink-0 border border-itec-border/50">
