  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 animate-in fade-in duration-500">
        ...
        <h3 className="text-itec-textfont-bold text-lg mb-1">Cargando material...</h3>
  ...
  if (safeCourses.length === 0) {
    return (
      <div className="bg-gradient-to-b from-slate-900 to-slate-950 border border-white/5 border-dashed rounded-3xl p-12 text-center shadow-2xl flex flex-col items-center justify-center relative overflow-hidden group animate-in fade-in slide-in-from-bottom-4">
        ...
        <h3 className="text-2xl font-bold text-itec-textmb-2 relative z-10 tracking-tight">Cinta vacía</h3>
          ...
          <div key={cursoId} className="relative group h-full">
            ...
            {isAdmin && !isSystemCourse && (
              <button 
                ...
                className="cursor-pointer absolute top-3 left-3 z-30 bg-red-600/90 hover:bg-red-500 text-itec-textw-9 h-9 rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-lg hover:scale-110 active:scale-95 border border-red-400/50"
