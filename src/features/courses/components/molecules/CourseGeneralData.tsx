...
export const CourseGeneralData: FC<Props> = ({
  ...
  return (
    <div className="space-y-4 mb-8">
      <h3 className="text-sm font-bold text-itec-textborder-b border-itec-gray pb-2">
        Detalles del Curso
      </h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-[11px] font-bold text-gray-500 uppercase mb-1">Título del Curso</label>
          <Input fullWidth placeholder="Ej: Clase de Paradigmas..." value={title} onChange={(e: any) => setTitle(e.target.value)} className="bg-itec-box border-itec-gray text-sm py-2.5" />
        </div>
        <div>
          <label className="block text-[11px] font-bold text-gray-500 uppercase mb-1">URL Portada (Opcional)</label>
          <Input fullWidth placeholder="https://..." value={image} onChange={(e: any) => setImage(e.target.value)} className="bg-itec-box border-itec-gray text-sm py-2.5" />
        </div>
        ...
        <div>
          ...
          {isCustomMateria ? (
            <div className="flex gap-2">
              <Input fullWidth placeholder="Escribe la materia..." value={materia} onChange={(e: any) => setMateria(e.target.value)} className="bg-itec-box border-itec-gray text-sm py-2.5 flex-1" />
              <button type="button" onClick={() => { setIsCustomMateria(false); setMateria(""); }} className="text-itec-text hover:text-itec-texttext-xs px-2">Volver</button>
            </div>
          ) : (
            <select
              value={materia}
              onChange={handleMateriaChange}
              className="w-full bg-itec-box border border-itec-gray text-itec-texttext-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-itec-blue transition-all"
            >
        ...
        <div className="md:col-span-2">
          ...
          <Input fullWidth placeholder="Ej: Repaso general para finales..." value={desc} onChange={(e: any) => setDesc(e.target.value)} className="bg-itec-box border-itec-gray text-sm py-2.5" />
        </div>
      </div>
    </div>
  );
