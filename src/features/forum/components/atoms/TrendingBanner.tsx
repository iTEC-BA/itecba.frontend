  ...
  if (view === 'form') {
    return (
      <div className="p-5 space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              className="w-10 h-10 rounded-lg cursor-pointer border border-itec-border/50 bg-transparent"
            />
            <input
              value={form.banner_color || '#b71234'}
              onChange={e => setForm(p => ({ ...p, banner_color: e.target.value }))}
              placeholder="#b71234"
              className="flex-1 bg-itec-card border border-itec-border/50 rounded-xl px-3 py-2.5 text-sm text-itec-text placeholder:text-itec-muted outline-none focus:border-itec-red/50 transition-colors font-mono"
            />
          </div>
        </div>
        <div>
        ...
        <div className="flex items-center justify-end gap-3 pt-2 border-t border-itec-border/50">
      ...
      {banners.length === 0 ? (
        ...
        <div className="space-y-2">
          {banners.map(b => (
            <div
              key={b.id}
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-itec-border/50 bg-itec-card"
            >
              {/* Color dot */}
...
export const TrendingBanner: React.FC = () => {
  ...
  return (
    <div className="relative">
      ...
      {/* Botón admin */}
      {isAdmin && (
        <div className="flex justify-end px-4 py-1.5 border-b border-itec-border/50 bg-itec-bg">
          <Button
            variant="secondary"
            hierarchy="ghost"
