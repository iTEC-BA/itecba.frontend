...
export const PostMoreMenu: React.FC<Props> = ({ postId, isAuthor, onDelete, onShare }) => {
  ...
  return (
    <div ref={menuRef} className="relative" onClick={stop}>
      ...
      {open && (
        <div className="absolute right-0 top-8 z-50 w-44 bg-itec-card border border-itec-border/50 rounded-xl shadow-2xl py-1 animate-in fade-in zoom-in-95 duration-150">
          <button
